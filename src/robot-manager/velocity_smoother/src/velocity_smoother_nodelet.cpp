...
#include <robot_manager_velocity_smoother/paramsConfig.h>
...
void VelocitySmoother::stopvelocityCB(const geometry_msgs::Twist::ConstPtr& msg)
{ 
   if (stop_period_record.size() < PERIOD_RECORD_SIZE)
  {
    stop_period_record.push_back((ros::Time::now() - last_stop_cb_time).toSec());
  }
  else
  {
    stop_period_record[stop_pr_next] = (ros::Time::now() - last_stop_cb_time).toSec();
  }

  stop_pr_next++;
  stop_pr_next %= stop_period_record.size();
  last_stop_cb_time = ros::Time::now();

  if (stop_period_record.size() <= PERIOD_RECORD_SIZE/2)
  {
    // wait until we have some values; make a reasonable assumption (10 Hz) meanwhile
    stop_cb_avg_time = stop_period_record.front();
  }
  else
  {
    // enough; recalculate with the latest input
    stop_cb_avg_time = median(stop_period_record);
  }

  stop_input_active = true;
  // ignore otherwise
}
...
void VelocitySmoother::spin()
{
  ...
  while (! shutdown_req && ros::ok())
  {
        ...
        (target_vel.angular.z != last_cmd_vel.angular.z))
    {
      ...
      if(stop_input_active) 
      {
        cmd_vel->linear.x = 0;
        cmd_vel->angular.z = 0;
        if((ros::Time::now() - last_stop_cb_time).toSec() > 2.0* stop_cb_avg_time) stop_input_active = false;
      }
...
bool VelocitySmoother::init(ros::NodeHandle& nh)
{
  ...
  stop_vel_sub    = nh.subscribe("e_stop",    1, &VelocitySmoother::stopvelocityCB, this);
