...
namespace JTCX
{
	...
	JtcxLogger* JtcxDir::_logger = JtcxSingleton<JtcxLogger>::GetInstance();
	...
	void JtcxDir::createDir(const std::string& dir)
	{
		...
		_logger->INFO("create directories: {}", dir);
	}

	void JtcxDir::deleteDir(const std::string& dir)
	{
		bfs::remove_all(bfs::path(dir));
		_logger->INFO("delete directories: {}", dir);
	}
	...
	std::string JtcxDir::getFatherDir(const std::string& path)
	{
