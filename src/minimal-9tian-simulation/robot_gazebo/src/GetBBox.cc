...
vector<string> Car = {"pickup", "ambulance", "hatch", "suv"};
vector<float> Car_yaw_offset = {0, -M_PI_2, 0, 0};
vector<string> Truck = {"bus", "truck"};
vector<float> Truck_yaw_offset = {-M_PI_2, -M_PI_2};
vector<string> Pedestrian = {"Scrubs", "walking", "FemaleVisitor", "MaleVisitorOnPhone"};
...
int getClassType(string str)
{
  ...
  {
    if (str.find(Pedestrian[i]) != str.npos) return Car.size() + Truck.size() + i;
  }
...
namespace gazebo
{
 ...
 void GetBBox::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
 {
   ...
   this->deferred_load_thread_ = boost::thread(
     boost::bind(&GetBBox::LoadThread, this));
 }
 ...
 bool GetBBox::ServiceCallback(robot_gazebo::bboxPerFrame::Request &req,
  ...
  if(req.msg == "bbox"){
    ...
    for (size_t i = 0; i < this->world_->ModelCount(); i++)
    {
      ...
      if(idx >= 0)
      {
        ...
        // ROS_INFO("(x, y, z) = (%f, %f, %f)", tmp_bbox.Center().X(), tmp_bbox.Center().Y(), tmp_bbox.Center().Z());
        ...
        double cx_in_world = cos(yaw) * tmp_bbox.Center().X() - sin(yaw) * tmp_bbox.Center().Y();
        double cy_in_world = sin(yaw) * tmp_bbox.Center().X() - cos(yaw) * tmp_bbox.Center().Y();
        ...
        bbox.pos.push_back(vis->Pose().Pos().Z() + tmp_bbox.Center().Z());
    ...
    if (this->scene_ == NULL) {
        ...
        if (this->scene_) {
            ...
            ros::AdvertiseServiceOptions aso =
                ros::AdvertiseServiceOptions::create<robot_gazebo::bboxPerFrame>(
                this->service_name_, boost::bind(&GetBBox::ServiceCallback,
